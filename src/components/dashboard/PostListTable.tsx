...
import { samplePosts } from "@/types/fakeData";
...
export default function PostManagement() {
    ...
    const [posts] = useState(samplePosts); // Replace 'posts' with actual posts data when available       
        ...
        return posts
            .filter(
                (post) =>
                    ...
                    post.post.toLowerCase().includes(searchText.toLowerCase()) ||
                    post.postUs.toLowerCase().includes(searchText.toLowerCase())
            )
            .sort((a, b) =>
                sortAsc ? a.userName.localeCompare(b.userName) : b.userName.localeCompare(a.userName)
            );
    }, [searchText, sortAsc, posts]);
                    ...
                    <TableBody>
                        {currentPosts.map((post, idx) => (
                            <TableRow key={post.id} className={idx % 2 === 0 ? "bg-white hover:bg-gray-50" : "bg-gray-50/50 hover:bg-gray-50"}>
                                ...
                                <TableCell className="py-3 px-4 text-gray-600">{post.date}</TableCell>
                                <TableCell className="py-3 px-4 text-gray-600">{post.post}</TableCell>
                                <TableCell className="py-3 px-4 text-gray-600">{post.postUs}</TableCell>
                                <TableCell className="py-4 px-6 border-0">
                                    <Badge
                                        className={`flex items-center justify-center w-[104px] h-[30px] gap-1 px-1 py-1 rounded-[6px] text-sm ${post.status === "Approved" ? "bg-[#52C41A33] text-green-800" : "bg-[#FF4D4F33] text-red-800"
                                            }`}
                                    >
                                        {post.status}
                                    </Badge>
